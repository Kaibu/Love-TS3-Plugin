...
void handle_client_movement(uint64 schid /* serverConnectionHandlerID */) {
...
void handle_client_unfollow(uint64 schid /* serverConnectionHandlerID */) {
	unfollow_on_server(schid);
}

void handle_client_follow(uint64 schid /* serverConnectionHandlerID */, uint64 clientID) {
	anyID myself;
	rote(ts3Functions.getClientID(schid, &myself));
...
const char* ts3plugin_author() {
    return "Christian Goltz, Stefan Seering, Bluscream, Kaibu";
}
